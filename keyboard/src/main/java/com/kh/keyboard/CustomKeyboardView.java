	...
	public void onDraw(Canvas canvas) {
		...
		try {
			...
			for (Keyboard.Key key : keys) {
				if (key.codes[0] == -5) {
					Drawable dr = (Drawable) context.getResources().getDrawable(R.drawable.keyboard_word_del_layerlist_small);
					dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
					dr.draw(canvas);
				} else if (key.codes[0] == -35) {
					Drawable dr = (Drawable) context.getResources().getDrawable(R.drawable.keyboard_word_del_layerlist_large);
					dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
					dr.draw(canvas);
	...
	private void drawText(Canvas canvas, Keyboard.Key key) {
		try {
			...
			if (key.label != null) {
				...
				canvas.drawText(key.label.toString(), key.x + (key.width / 2), (key.y + key.height / 2) + bounds.height() / 2,
						paint);
